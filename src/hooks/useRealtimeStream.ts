/**
 * SSE client hook for the Super Admin dashboard.
 *
 * PA8 — instant push-based delivery for Chat and Notifications.
 *
 * Opens a single `EventSource` to `/api/realtime/stream` (which
 * proxies to the tenantapp's SSE endpoint). The hook:
 *
 ...
 *
 * The hook is mounted once by the dashboard layout (or by the
 * `NotificationBell` component) and shared across the admin app.
 * Multiple components can register handlers for different event
 * kinds; the hook multiplexes them onto a single SSE connection.
 *
 * Usage:
 *
