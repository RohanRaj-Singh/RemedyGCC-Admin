...
/**
 * Super Admin Invoices — List
 *
 * Proxies to the Tenant App's `/api/invoices` list endpoint with the shared
 * API key. Optional `tenantId`/`status` filters are forwarded.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const targetUrl = new URL(`${tenantAppUrl}/api/invoices`);
