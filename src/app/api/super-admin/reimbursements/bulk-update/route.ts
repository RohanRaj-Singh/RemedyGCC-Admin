...
/**
 * Super Admin Bulk Progress Update (FR-046)
 *
 * Proxies to the Tenant App's `/api/admin/reimbursements/bulk-update` endpoint
 * with the shared API key. Body: `{ tenantId, claimIds, message }`. Posts a
 * progress update to every selected claim belonging to the supplied tenant.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${tenantAppUrl}/api/admin/reimbursements/bulk-update`, {
      method: 'POST',
