...
export function unlockAudio(): void {
  const a = getAudio();
  if (!a) return;
  // Play+pause on the first user gesture unlocks the Audio() instance for
  // subsequent silent .play() calls (browser autoplay policy).
  a.play().then(() => {
    a.pause();
    a.currentTime = 0;
  }).catch(() => {
...
export function playNotificationSound(volume: number = DEFAULT_VOLUME): void {
  ...
  a.play().catch(() => {
    ...
    // The user's next click will call unlockAudio() via the settings toggle.
  });
}
